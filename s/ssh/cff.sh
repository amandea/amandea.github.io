...
if [[ "${#RECORD}" -le 10 ]]; then
     ...
     --data '{"type":"A","name":"'${SUB_DOMAIN}'","content":"'${IPP}'","ttl":120,"proxied":true}' | jq -r .result.id)
fi
...
RESULT=$(curl -sLX PUT "https://api.cloudflare.com/client/v4/zones/${ZONE}/dns_records/${RECORD}" \
     ...
     -H "X-Auth-Key: ${CF_KEY}" \
     -H "Content-Type: application/json" \
     --data '{"type":"A","name":"'${SUB_DOMAIN}'","content":"'${IPP}'","ttl":120,"proxied":true}')
